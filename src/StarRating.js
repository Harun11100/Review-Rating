...
StarRating.propTypes={
      ...
      // onUserSetRating:PropTypes.func
      ...
      return(<div style={containerStyle}>
            <div style={starContainer}>
                  {Array.from({length:maxrating },(_,i)=><Star 
                        ...
                        size={size}
                        key={i} 
                        onRate={()=>ratetingHandler(i+1)} 
                        full={temRating?temRating>=i+1:rate>=i+1}
                        onHoverIn={()=>setTemRating(i+1)} 
                        onHoverOut={()=>setTemRating(0)}
                        /> )}
                  
            </div>
            <p style={textStyle}>{massage.length===maxrating?
             massage[temRating?temRating-1:rate-1]
             :temRating||rate||""}
             </p>
      </div>
)}
